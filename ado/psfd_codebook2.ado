...
! if not exist ".\documents\rawdata\" mkdir ".\documents\rawdata\"
	...
	* export the table to Word (需使用Stata 17)
	...
	putdocx table tb1(2,1) = ("中華民國行業標準分類：第11次修訂"), linebreak(1)
	putdocx table tb1(2,1) = ("（民國110年1月）"), append
...
local pdata `anything'
	...
	while n[`i']==1 {
		...
		putdocx table tab1(`p',6) = (remark[`i']), rowspan(`s')
		continue, break
		}
	}
	quietly cd "..\"
